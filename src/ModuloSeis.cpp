...
int main()
{
	...
	GLFWwindow* window = glfwCreateWindow(WIDTH, HEIGHT, "Olá, Módulo Seis -- Arthur Juchem!", nullptr, nullptr);
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		for (int i = 0; i < objects.size(); i++) {
            if (objects[i].isMoving && objects[i].pathPoints.size() >= 2) {
                
                glm::vec3 target = objects[i].pathPoints[(objects[i].currentTarget + 1) % objects[i].pathPoints.size()];
                ...
                if (distance <= step) {
                    objects[i].position = target;
                    objects[i].currentTarget = (objects[i].currentTarget + 1) % objects[i].pathPoints.size();
                } else {
                    objects[i].position += glm::normalize(direction) * step;
                }
            }
